...
fn evaluate_reports(report_vec: Vec<String>) -> Vec<bool> {
    ...
    for l in report_vec {
        ...
        let mut report_eval_permutations: Vec<bool> = vec![];
        for (i, ri) in report_as_int.iter().enumerate() {
            let mut current_report_with_removed_element = report_as_int.clone();
            current_report_with_removed_element.remove(i);
            report_eval_permutations.push(evaluate_report(current_report_with_removed_element));
        }

        if report_eval_permutations.into_iter().filter(|b| *b).count() >= 1 {
            ret_vec.push(true);
        } else {
            ret_vec.push(false);
        }
...
// println!("{}", x.iter().map(|&n| n.to_string()).collect::<Vec<String>>().join(" "));
